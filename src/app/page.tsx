...
import {
  ...
  Stethoscope, SmilePlus, HeartPulse, Shield, CheckCircle2, Star, Megaphone,
} from "lucide-react";
...
const iconColors = ["#F7A5A5","#FFDBB6","#5D688A","#F7A5A5","#FFDBB6","#5D688A","#F7A5A5","#FFDBB6"];
...
async function getSettings() {
  try {
    const raw = await gsheet.call("settings_get") as Record<string, unknown> | null;
    if (!raw || (raw as Record<string, unknown>).error) return null;
    if (raw.services && typeof raw.services === "string") {
      try { raw.services = JSON.parse(raw.services as string); } catch { raw.services = []; }
    }
    return raw;
  } catch { return null; }
}
    ...
    <div className="min-h-screen bg-mesh overflow-x-hidden">
      ...
      <section className="relative min-h-[88vh] sm:min-h-[92vh] flex items-center overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20 grid lg:grid-cols-2 gap-8 lg:gap-12 items-center w-full">
          {/* Left */}
          <div className="z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs font-semibold mb-5 sm:mb-6 glass"
          ...
          <div className="flex justify-center items-center mt-4 lg:mt-0">
            <div className="relative w-full max-w-xs sm:max-w-sm lg:max-w-md glass rounded-3xl px-6 py-8 sm:px-8 sm:py-10 text-center"
              ...
              <div className="relative mx-auto mb-5 sm:mb-6" style={{ width:160, height:160 }}>
                ...
                <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"
                  ...
                  <g>
                    <path style={{ fill:"#5D688A" }} d="M164.25,512c-49.81,0-69.856-77.647-84.493-134.341c-3.58-13.868-6.963-26.968-9.569-33.223C40.794,273.92,23.246,200.972,23.246,149.297c0-38.541,13.133-75.119,36.981-102.995C85.773,16.444,120.995,0,159.409,0c30.782,0,61.291,17.624,83.565,30.493l0.311,0.179c7.812,4.514,17.628,4.519,25.532-0.036l0.31-0.178C291.45,17.602,322.012,0,352.591,0c38.414,0,73.636,16.444,99.179,46.303c0.009,0.011,0.019,0.02,0.028,0.031c23.832,27.874,36.956,64.438,36.956,102.965c0,17.872-4.459,45.411-14.031,86.662c-1.964,8.462-10.42,13.725-18.877,11.766c-8.462-1.964-13.729-10.415-11.765-18.876c8.893-38.327,13.217-64.348,13.217-79.553c0-31.035-10.445-60.342-29.409-82.522c-0.006-0.008-0.014-0.016-0.02-0.024c-19.472-22.762-46.205-35.296-75.276-35.296c-22.169,0-48.519,15.175-67.758,26.255l-0.315,0.181c-17.597,10.142-39.461,10.131-57.05-0.031c-0.041-0.024-0.187-0.107-0.228-0.131c-19.197-11.089-45.483-26.274-67.832-26.274c-29.071,0-55.804,12.534-75.278,35.295c-18.976,22.182-29.428,51.498-29.428,82.546c0,47.733,16.642,116.158,44.519,183.037c3.47,8.324,6.788,21.183,10.991,37.462c6.177,23.923,13.863,53.695,24.354,77.021c6.35,14.116,13.02,24.116,19.477,29.396c0.972-3.995,2.042-8.506,3.097-12.956c7.043-29.719,17.687-74.63,32.199-113.235c12.632-33.599,33.36-78.243,67.406-78.243c0.05,0,0.094,0,0.145,0c34.084,0.127,54.453,44.889,66.811,78.546c14.171,38.598,24.418,83.414,31.201,113.072c0.994,4.346,2.001,8.75,2.921,12.668c8.419-6.872,15.458-20.417,19.522-29.521c10.453-23.421,18.047-53.387,24.149-77.466c4.201-16.583,7.52-29.681,11.043-38.154c3.334-8.022,12.542-11.822,20.56-8.487c8.023,3.334,11.822,12.539,8.487,20.56c-2.641,6.353-6.021,19.689-9.599,33.807c-14.443,56.995-34.222,135.052-84.515,135.052c-13.007,0-16.208-12.38-18.121-19.777c-1.373-5.312-3.025-12.531-5.113-21.669c-6.604-28.881-16.586-72.524-30.065-109.242c-20.651-56.248-35.746-57.926-37.397-57.933c-1.653,0.061-16.826,1.568-37.987,57.855c-13.811,36.734-24.178,80.476-31.035,109.42c-2.177,9.187-3.898,16.444-5.324,21.785C180.458,499.667,177.166,512,164.25,512z"/>
                    <path style={{ fill:"#5D688A" }} d="M256,159.795c-49.963,0-94.468-20.875-110.746-51.945c-4.031-7.694-1.062-17.201,6.633-21.232c7.691-4.03,17.201-1.062,21.232,6.633c10.823,20.658,44.906,35.087,82.881,35.087s72.057-14.428,82.881-35.087c4.03-7.694,13.533-10.665,21.23-6.633c7.696,4.031,10.664,13.536,6.633,21.232C350.467,138.92,305.961,159.795,256,159.795z"/>
                  </g>
                  <path style={{ fill:"#F7A5A5" }} d="M83.585,335.665c0.374,0.908,0.741,1.815,1.118,2.721c12.738,30.565,29.936,157.887,79.545,157.887c3.592,0,20.156-98.954,47.98-160.608H83.585z"/>
                  <path style={{ fill:"#5D688A" }} d="M164.25,512c-49.961,0-69.932-77.632-84.512-134.318c-3.57-13.88-6.944-26.992-9.55-33.247l-1.145-2.785c-1.998-4.852-1.442-10.381,1.477-14.739s7.82-6.976,13.067-6.976h128.643c5.34,0,10.316,2.71,13.212,7.196c2.896,4.486,3.32,10.135,1.123,15.003c-17.666,39.148-30.905,95.004-38.815,128.379c-2.177,9.187-3.898,16.444-5.324,21.785C180.458,499.667,177.166,512,164.25,512z"/>
                  <path style={{ fill:"#FFDBB6" }} d="M300.865,335.665c27.295,61.611,43.041,160.465,46.605,160.465c50.084,0,66.763-128.308,79.589-159.169l0.404-1.296H300.865z"/>
                  <path style={{ fill:"#5D688A" }} d="M347.47,511.858c-13.007,0-16.208-12.38-18.121-19.777c-1.373-5.312-3.025-12.531-5.115-21.669c-7.631-33.366-20.403-89.215-37.75-128.376c-2.156-4.865-1.705-10.489,1.197-14.948c2.903-4.459,7.863-7.15,13.184-7.15h126.598c5.002,0,9.704,2.378,12.668,6.406c2.965,4.028,3.836,9.225,2.35,14l-0.404,1.298c-0.145,0.461-0.308,0.914-0.492,1.361c-2.641,6.353-6.021,19.689-9.599,33.807C417.544,433.801,397.765,511.858,347.47,511.858z"/>
                </svg>
              </div>

              <h3 className="font-extrabold text-[#3a3f52] text-lg sm:text-xl mb-1">drg. {doctorName}</h3>
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass text-xs font-semibold text-[#5D688A]"
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="relative rounded-3xl overflow-hidden text-center px-6 py-10 sm:p-12"
            ...
            <div className="absolute top-4 right-8 w-20 h-20 rounded-full"
              style={{ background:"rgba(255,219,182,0.15)", border:"1px solid rgba(255,255,255,0.2)" }} />
            <div className="absolute bottom-4 left-8 w-12 h-12 rounded-full"
              style={{ background:"rgba(247,165,165,0.15)", border:"1px solid rgba(255,255,255,0.2)" }} />
