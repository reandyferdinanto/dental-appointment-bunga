...
const sidebarLinks = [
  ...
  { href: "/dashboard/schedules", label: "Kelola Jadwal", icon: CalendarClock },
    ...
    <>
      ...
      <div className="p-5" style={{ borderBottom: "1px solid rgba(93,104,138,0.12)" }}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-bold text-sm text-[#3a3f52] block">drg. Bunga Maureen</span>
            <span className="text-[10px] font-medium" style={{ color: "#F7A5A5" }}>Dashboard Koas ✨</span>
          </div>
          {isMobile && (
            <button onClick={() => setSidebarOpen(false)} className="ml-auto p-1.5 rounded-lg hover:bg-white/50 text-[#5D688A]">
              <X className="w-5 h-5" />
            </button>
          )}
        </div>
      ...
      <div className="p-3" style={{ borderTop: "1px solid rgba(93,104,138,0.12)" }}>
        <Link href="/"
          className="block px-4 py-2.5 rounded-2xl text-sm font-medium text-[#5D688A]/60 hover:text-[#5D688A] hover:bg-white/50 transition-all mb-1"
  ...
  return (
    <div className="min-h-screen bg-mesh">
      ...
      <div className="lg:hidden sticky top-0 z-50 flex items-center justify-between px-4 py-3"
        style={{ background: "rgba(255,242,239,0.85)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(247,165,165,0.2)" }}>
        <button onClick={() => setSidebarOpen(true)} className="p-2 rounded-xl text-[#5D688A] hover:bg-white/50 transition-colors">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-72 flex flex-col"
            style={{ background: "rgba(255,242,239,0.96)", backdropFilter: "blur(20px)", borderRight: "1px solid rgba(255,255,255,0.6)" }}>
      ...
      <div className="flex">
        ...
        <aside className="hidden lg:flex flex-col fixed left-0 top-0 bottom-0 w-64 z-40"
          style={{ background: "rgba(255,242,239,0.85)", backdropFilter: "blur(20px)", borderRight: "1px solid rgba(255,255,255,0.6)", boxShadow: "4px 0 24px rgba(93,104,138,0.08)" }}>
          <SidebarContent />
        </aside>
